...
var pollingLiscenseExpNotif = function(){
    ...
    var query = db.query('SELECT id,user_id FROM  `liscenses` WHERE  DATEDIFF(expirydate,"'+today+'") = 1 AND unseenstatus = 0');
...
var updateManagerNotifi = function(data)
{
    ...
    data.users.forEach(function(i)
        {
            var url = 'http://localhost/shiftmate_api/BoardUsers/getBoardManager/'+i.board_id+'.json';
            requestify.get(url)
